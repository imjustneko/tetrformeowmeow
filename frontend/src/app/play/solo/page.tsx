  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-[#050508] text-white">
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse 120% 80% at 50% 100%, rgba(120,40,140,0.35), transparent 55%), linear-gradient(180deg, #0a0a12 0%, #12101c 40%, #1a1028 100%)',
        }}
      />
      <Navbar />
        ...
        {!isActive && !isFinished && (
          <div className="text-center">
            <h1 className="mb-4 text-3xl font-black tracking-tight sm:text-4xl">
              <span className="text-cyan-400">Solo</span> practice
            </h1>
            <p className="mb-8 text-sm text-zinc-400 sm:text-base">No timer — stack at your pace</p>
                ...
                <div className="relative flex w-max max-w-full shrink-0 flex-row items-stretch overflow-hidden rounded-sm shadow-lg shadow-black/30">
                  ...
                  {isFinished && finalState ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/75 backdrop-blur-[2px]">
                      <h2 className="mb-2 text-2xl font-black text-red-400 sm:text-3xl">Game over</h2>
                      <p className="mb-1 text-sm text-zinc-300 sm:text-base">
                        Score:{' '}
                        <span className="font-bold text-white">{finalState.score.toLocaleString()}</span>
                      </p>
                      <p className="mb-6 text-sm text-zinc-300 sm:text-base">
                        Lines: <span className="font-bold text-white">{finalState.lines}</span>
                      </p>
